<template>
    <div class="executable-settings q-pa-md">
        <div class="row justify-between">
            ...
            <QCheckbox v-if="type != 'pc98'" :modelValue="withAppLocale" @update:modelValue="updateWithAppLocale" label="With Applocale"></QCheckbox>
        </div>
        <div class="row justify-between">
            <PostponedInput class="path-input col-7 q-pb-none" v-model="path" :show-toolip="true"></PostponedInput>
            <QBtn class="col-2 fit-content" color="white" text-color="black" label="Browse" @click="browse"></QBtn>
            <QBtn class="col-2 fit-content" color="white" text-color="black" label="Launch" @click="launch"></QBtn>
        </div>
    </div>
    <ErrorPopup v-model:visible="showError" :showCloseX="true" :error="'The path is not set.'" :errorTitle="`Cannot run ${title}`"></ErrorPopup>
</template>
...
const titles: Record<ConfigurableFile, string> = {
